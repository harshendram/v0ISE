"""
Multi-Agent Medical Assistant - Main Application
Author: harshendram
Year: 2025
License: Apache 2.0

FastAPI-based medical AI assistant with multi-agent orchestration,
RAG capabilities, and computer vision analysis for medical imaging.
"""
...
app = FastAPI(
    ...
    version="2.0",
    description="AI-powered medical assistant with multi-agent architecture"
)

# Directory configuration
BACKEND_UPLOAD_DIR = "uploads/backend"
FRONTEND_UPLOAD_DIR = "uploads/frontend"
SKIN_ANALYSIS_OUTPUT_DIR = "uploads/SKIN_ANALYSIS_OUTPUT_DIR"
AUDIO_OUTPUT_DIR = "uploads/speech"

# Ensure all directories exist
for dir_path in [BACKEND_UPLOAD_DIR, FRONTEND_UPLOAD_DIR, SKIN_ANALYSIS_OUTPUT_DIR, AUDIO_OUTPUT_DIR]:
...
tts_client = ElevenLabs(
    api_key=app_config.speech.eleven_labs_api_key,
)
...
def is_valid_image_file(filename):
    ...
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in SUPPORTED_IMAGE_FORMATS

def cleanup_audio_files():
    """Background task: removes old audio files every 5 minutes."""
    while True:
        try:
            audio_files = glob.glob(f"{AUDIO_OUTPUT_DIR}/*.mp3")
            for audio_file in audio_files:
                os.remove(audio_file)
            print(f"Cleaned up {len(audio_files)} audio files.")
        except Exception as error:
            print(f"Audio cleanup error: {error}")
        time.sleep(300)  # 5-minute interval

# Start background cleanup service
audio_cleanup_thread = threading.Thread(target=cleanup_audio_files, daemon=True)
audio_cleanup_thread.start()
...
def chat(
    ...
    try:
        ...
        if response_data["agent_name"] == "SKIN_LESION_AGENT, HUMAN_VALIDATION":
            segmentation_path = os.path.join(SKIN_ANALYSIS_OUTPUT_DIR, "segmentation_plot.png")
            if os.path.exists(segmentation_path):
                result["result_image"] = f"/uploads/SKIN_ANALYSIS_OUTPUT_DIR/segmentation_plot.png"
            else:
...
async def upload_image(
    ...
    if len(file_content) > app_config.api.max_image_upload_size * 1024 * 1024:  # Convert MB to bytes
        return JSONResponse(
            status_code=413, 
            content={
                "status": "error",
                "agent": "System",
                "response": f"File too large. Maximum size allowed: {app_config.api.max_image_upload_size}MB"
            }
        )
    ...
    file_path = os.path.join(BACKEND_UPLOAD_DIR, filename)
...
async def transcribe_audio(audio: UploadFile = File(...)):
    ...
    try:
        ...
        temp_audio = f"./{AUDIO_OUTPUT_DIR}/speech_{uuid.uuid4()}.webm"
        ...
        # Convert to MP3
        mp3_path = f"./{AUDIO_OUTPUT_DIR}/speech_{uuid.uuid4()}.mp3"
        ...
        try:
            ...
            transcription = tts_client.speech_to_text.convert(
                file=audio_data,
                model_id="scribe_v1",
...
async def generate_speech(request: SpeechRequest):
    ...
    try:
        ...
        headers = {
            ...
            "xi-api-key": app_config.speech.eleven_labs_api_key
        }
        payload = {
            "text": text,
        ...
        # Save the audio file temporarily
        os.makedirs(AUDIO_OUTPUT_DIR, exist_ok=True)
        temp_audio_path = f"./{AUDIO_OUTPUT_DIR}/{uuid.uuid4()}.mp3"
